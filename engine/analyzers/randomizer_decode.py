...
def _decode_randomizer_text(text: str) -> Tuple[str, List[Dict[str, str]], float, float]:
    ...
    for part in parts:
        ...
        # Encodings such as padded Base64 use punctuation as part of the token.
        # Try the complete whitespace-delimited token before splitting it into
        # word/punctuation fragments for Caesar and similar transforms.
        whole_name, whole_decoded, _ = _decode_word(part)
        if whole_name != "raw" and whole_decoded != part:
            decoded_parts.append(whole_decoded)
            if len(transforms) < MAX_TRANSFORMS:
                transforms.append(
                    {"token": part, "transform": whole_name, "decoded": whole_decoded}
                )
            continue
