...
def _length_prefixed_blob(blob: bytes) -> bytes:
    """Return the encoder's 4-byte-length-prefixed zlib stream, if valid."""
    if len(blob) < 5:
        return b""
    length = int.from_bytes(blob[:4], "big")
    if length <= 0 or length > MAX_BYTES or length > len(blob) - 4:
        return b""
    ...
    try:
        ...
        for plane in planes:
            try:
                ...
                # encode_zlib_into_image writes a four-byte compressed-length prefix.
                # Prefer the exact framed stream so trailing carrier bits cannot
                # confuse decompression.
                framed = _length_prefixed_blob(data_full)
                if framed:
                    candidates.append(("length_prefix", framed))

                candidates.append(("full", data_full))
